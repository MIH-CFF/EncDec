...
def key_transformation(seed,key, target_length,val):
    print(val)
    large_key_list = list(key[::-1]+'10'+key[3:])
    if(val<128):
        for i in range (0,val):
            large_key_list = large_key_list[1:] + [large_key_list[0]]
    else:
        for i in range (0,val):
            large_key_list = [large_key_list[-1]] + large_key_list[:-1]
    ...
    for i in range(0,1000):
        segment=''.join(large_key_list[i*8:(i+1)*8])
...
def encryption(TransformedKeyBits, image_path):
    ...
    for i in range(height):
        for j in range(width):
            ...
            for k in range(channels):
                if index < len(TransformedKeyBits):                    
                    rule=(index%8)+1
                    key_segment = TransformedKeyBits[index*8:(index+1)*8]
...
def run():
    ...
    def convert(pw):
        ...
        val=0
        val+=int(image_array[image_row-1,image_col-1,0])
        val+=int(image_array[image_row-1,image_col-1,1])
        val+=int(image_array[image_row-1,image_col-1,2])
        val=val//3
        transformed_key= key_transformation(seed,binary_string, target_length,val)
